...
# nakakoutv
proc quad_fill Vec2 p1, Vec2 p2, Vec2 p3, Vec2 p4 {
    if ($p2.y - $p1.y) * ($p3.x - $p2.x) < ($p2.x - $p1.x) * ($p3.y - $p2.y) {
        quad_fill_ccw $p1, $p2, $p3, $p4;
    } else {
        quad_fill_ccw $p4, $p3, $p2, $p1;
    }
}
# using a massive macro instead of an inner function. This allows for using local vars only and is slightly faster, but is horrible to code in
# this can be optimised to use inputted v1,v2,v3,v4 instead of recalculating them
%define _QUAD_FILL_INNER(p1, p2, p3, p4, u1, u2, u3, u4) \
    v4 = V2_SUB(p4, p1);\
    local Vec2 e1 = Vec2(u1.x * tri_thick + p1.x, u1.y * tri_thick + p1.y);\
    local Vec2 e2 = Vec2(u2.x * tri_thick + p2.x, u2.y * tri_thick + p2.y);\
    calc1 = p4.x * p1.y - p4.y * p1.x;\
    calc2 = V2_MAG(v4);\
    if abs(e2.x * v4.y - e2.y * v4.x + calc1) / calc2 < tri_thick {\
        v3 = V2_SUB(p3, p4);\
        v2 = V2_SUB(p2, p3);\
        local calc3 = 1 + (v2.x * v3.y - v3.x * v2.y) / (v2.x * (p1.y - p3.y) - v2.y * (p1.x - p3.x));\
        local Vec2 f0 = Vec2(p1.x + v4.x / calc3, p1.y + v4.y / calc3);\
        local Vec2 f3 = V2_SUB(f0, p3);\
        local Vec2 f4 = V2_SUB(p4, f0);\
        local fd3 = V2_MAG(f3);\
        local fd4 = V2_MAG(f4);\
        local g0x = f4.y * f3.x - f4.x * f3.y;\
        local Vec2 g0 = Vec2((f3.x * fd4 - f4.x * fd3) / g0x, (f3.y * fd4 - f4.y * fd3) / g0x);\
        local s0 = V2_MAG2(g0);\
        fill_ratio = MAX(fill_ratio, s0);\
        fill_ratio = 0.5 - 0.5 / sqrt(fill_ratio);\
        local Vec2 e0 = Vec2(g0.x * tri_thick + f0.x, g0.y * tri_thick + f0.y);\
        pen_up;\
        set_pen_size 2 * tri_thick;\
        V2_GOTO(e0);\
        pen_down;\
        goto u3.x * tri_thick + p3.x, u3.y * tri_thick + p3.y;\
        goto u4.x * tri_thick + p4.x, u4.y * tri_thick + p4.y;\
        V2_GOTO(e0);\
        tri_thick *= fill_ratio;\
        e2 = Vec2(u2.x * tri_thick + p2.x, u2.y * tri_thick + p2.y);\
        e0 = Vec2(g0.x * tri_thick + f0.x, g0.y * tri_thick + f0.y);\
        until (abs(e2.x * v4.y - e2.y * v4.x + calc1) / calc2) >= tri_thick or tri_thick < fill_ratio {\
            pen_up;\
            set_pen_size 2 * tri_thick;\
            V2_GOTO(e0);\
            pen_down;\
    ...
    until tri_thick < fill_ratio {\
        pen_up;\
        set_pen_size 2 * tri_thick;\
        goto u1.x * tri_thick + p1.x, u1.y * tri_thick + p1.y;\
        pen_down;\
        goto u2.x * tri_thick + p2.x, u2.y * tri_thick + p2.y;\
        goto u3.x * tri_thick + p3.x, u3.y * tri_thick + p3.y;\
        goto u4.x * tri_thick + p4.x, u4.y * tri_thick + p4.y;\
        goto u1.x * tri_thick + p1.x, u1.y * tri_thick + p1.y;\
        tri_thick *= fill_ratio;\
...
proc quad_fill_ccw Vec2 p1, Vec2 p2, Vec2 p3, Vec2 p4 {
    ...
    # v3 = p3 <- p4
    # v4 = p4 <- p1
    local Vec2 v1 = V2_SUB($p1, $p2);
    local Vec2 v2 = V2_SUB($p2, $p3);
    local Vec2 v3 = V2_SUB($p3, $p4);
    local Vec2 v4 = V2_SUB($p4, $p1);
    ...
    local a1 = v4.y * v1.x - v4.x * v1.y;
    local Vec2 u1 = Vec2(
        (v1.x * d4 - v4.x * d1) / a1,
        (v1.y * d4 - v4.y * d1) / a1
    );

    local a2 = v1.y * v2.x - v1.x * v2.y;
    local Vec2 u2 = Vec2(
        (v2.x * d1 - v1.x * d2) / a2,
        (v2.y * d1 - v1.y * d2) / a2
    );

    local a3 = v2.y * v3.x - v2.x * v3.y;
    local Vec2 u3 = Vec2(
        (v3.x * d2 - v2.x * d3) / a3,
        (v3.y * d2 - v2.y * d3) / a3
    );

    local a4 = v3.y * v4.x - v3.x * v4.y;
    local Vec2 u4 = Vec2(
        (v4.x * d3 - v3.x * d4) / a4,
        (v4.y * d3 - v3.y * d4) / a4
    );

    local calc2 = v2.x * v4.y > v2.y * v4.x;
    if calc2 {
        if u2.x == u1.x {
            local r2 = v1.y / ((u2.y - u1.y) * 2);
        } else {
            local r2 = v1.x / ((u2.x - u1.x) * 2);
        }
    } else {
        if u4.x == u3.x {
            local r2 = v3.y / ((u4.y - u3.y) * 2);
        } else {
            local r2 = v3.x / ((u4.x - u3.x) * 2);
        }
    }
    local calc1 = v1.x * v3.y < v1.y * v3.x;
    if calc1 {
        if u3.x == u2.x {
            local r1 = v2.y / ((u3.y - u2.y) * 2);
        } else {
            local r1 = v2.x / ((u3.x - u2.x) * 2);
        }
        ...
        local fill_ratio = s1;
        fill_ratio = MAX(fill_ratio, s2);
        fill_ratio = MAX(fill_ratio, s3);
        fill_ratio = MAX(fill_ratio, s4);

        if r1 < r2 {
            local tri_thick = r1;
            _QUAD_FILL_INNER($p4, $p1, $p2, $p3, u4, u1, u2, u3);
        } else {
            local tri_thick = r2;
            if calc2 {
                _QUAD_FILL_INNER($p3, $p4, $p1, $p2, u3, u4, u1, u2);
            } else {
                _QUAD_FILL_INNER($p1, $p2, $p3, $p4, u1, u2, u3, u4);
            }
        }

    } else {
        if u1.x == u4.x {
            local r1 = v4.y / ((u1.y - u4.y) * 2);
        } else {
            local r1 = v4.x / ((u1.x - u4.x) * 2);
        }
        local s1 = V2_MAG2(u1);
        local s2 = V2_MAG2(u2);
        local s3 = V2_MAG2(u3);
        ...
        if r1 < r2 {
            local tri_thick = r1;
            _QUAD_FILL_INNER($p2, $p3, $p4, $p1, u2, u3, u4, u1);
        } else {
            local tri_thick = r2;
            if calc1 {
                _QUAD_FILL_INNER($p3, $p4, $p1, $p2, u3, u4, u1, u2);
...
proc draw_crescent Circle c1, Circle c2, res=30 {
    ...
    if isct.x1 == CircIntersectCases.notouch {
        ...
        if d1 < d2 {
            draw_arc_edge POS_CIRCD($c2, 180 + d1), -360 + (d2 - d1), $res;
        } else {
            draw_arc_edge POS_CIRCD($c2, 180 + d1), d2 - d1, $res;
        }
        ...
        if d1 < d2 {
            draw_arc_edge POS_CIRCD($c1, 180 + d1), -360 + (d2 - d1), $res;
        } else {
            draw_arc_edge POS_CIRCD($c1, 180 + d1), d2 - d1, $res;
        }
    }
}
...
proc fill_miter Line2 l1, Line2 l2, th {
    # Assume l2.x1, l2.y1 to be equal to l1.x2, l1.y2
    local dst1 = LINE2_LENGTH($l1);
    local v1x = (LINE2_DX($l1) / dst1) * (($th - 1) / 2);
    local v1y = (LINE2_DY($l1) / dst1) * (($th - 1) / 2);

    local dst2 = LINE2_LENGTH($l2);
    local v2x = (LINE2_DX2($l2) / dst2) * (($th - 1) / 2);
    local v2y = (LINE2_DY2($l2) / dst2) * (($th - 1) / 2);
    ...
    if cmp1 <= 0 or cmp2 >= 0 {
        ...
        local Vec2 ints = line2_intersect(
            Line2(
                ($l1.x1 + v1y),
                ($l1.y1 - v1x),
                ($l1.x1 + v1y) + v1x,
...
proc fill_miter_arc Line2 l1, Line2 l2, th {
    # Assume l2.x1, l2.y2 to be equal to l1.x2, l1.y2
    local dst1 = LINE2_LENGTH($l1);
    local v1x = (LINE2_DX($l1) / dst1) * ($th / 2);
    local v1y = (LINE2_DY($l1) / dst1) * ($th / 2);

    local dst2 = LINE2_LENGTH($l2);
    local v2x = (LINE2_DX2($l2) / dst2) * ($th / 2);
    local v2y = (LINE2_DY2($l2) / dst2) * ($th / 2);
